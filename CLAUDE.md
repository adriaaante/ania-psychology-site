...
## Что это
Премиум сайт-визитка **Анны Маловичко** (Пятигорск). ВАЖНО: Анна — ещё
СТУДЕНТКА-психолог, права на частную практику пока НЕТ. Поэтому существуют
ДВЕ версии сайта:
- **Прод (main) — «студенческий» режим**: личная страница «будущий психолог»,
  без услуг/цен/записи/отзывов/цифр/кабинета/часов. services.html — редирект
  на главную. Все формулировки — без обещания услуг («напишите мне», не
  «запишитесь»). Дисклеймер в футере. Скрытые блоки помечены комментарием
  «Полная версия блока — в ветке site-full-version».
- **Ветка `site-full-version` — полная версия психолога** (та, что была
  показана заказчику): услуги, цены, запись, отзывы, кабинет, сертификаты.
  ВЕТКУ НЕ ТРОГАТЬ и НЕ УДАЛЯТЬ. После получения Анной диплома: перенести
  актуальные наработки (фото, календарь, бейдж и пр. уже в ней есть до
  коммита e4b89e1) и вернуть полную версию в main.
Студия сертификатов (gift-studio) в коде осталась, нигде не афишируется —
НЕ использовать до диплома (сертификаты = продажа услуг).
