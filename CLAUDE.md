...
ВАЖНО про статус и ДВЕ версии сайта. Анна — ещё студентка, права на частную
практику пока НЕТ (сама сообщила). Сейчас она НЕ планирует развивать сайт —
доучивается. По решению заказчика на проде **полная версия** (услуги, цены,
запись, отзывы) — «для красоты», сайт не продвигается и никому не рассылается.
- Ветка **`site-student-version`** — «студенческий» режим: личная страница
  «будущий психолог», без услуг/цен/записи/отзывов/цифр/кабинета/часов,
  services.html → редирект, дисклеймер в футере, schema.org Person.
  Готова к мгновенной публикации, если сайт начнут показывать людям
  или продвигать (тогда полная версия = обещание услуг без квалификации).
- Ветка **`site-full-version`** — снимок полной версии (то же, что на main).
  НЕ удалять обе ветки.
Переключение: `git revert` соответствующего коммита либо merge нужной ветки
в main (Pages деплоит main). Студия сертификатов до диплома НЕ используется.
